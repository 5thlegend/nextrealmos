    ...
    <div className="space-y-6 max-w-5xl">
            ...
            {allRealms.map((r) => (
              <li key={r.id} className="py-3 flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  {ownedIds.has(r.id) && <Badge variant="accent">YOURS</Badge>}
      ...
      <Panel eyebrow={`// your memberships · ${mine.length}`} title="Realms you're active in">
        {mine.length === 0 ? (
          ...
          <ul className="divide-y divide-border/40">
            {mine.map((m: any) => (
              <li key={m.realm_id} className="py-2 flex items-center justify-between">
                <span className="font-medium">{m.realms?.name ?? m.realm_id}</span>
                <div className="flex items-center gap-3 text-xs">
                  <span className="font-mono text-muted-foreground">{m.realm_xp ?? 0} realm XP</span>
                  {m.last_active_at && <span className="text-muted-foreground">{relativeTime(m.last_active_at)}</span>}
                </div>
              </li>
            ))}
          </ul>
